...
type SupplierDetail = SupplierSummary & {
  notes?: string;
  // optional fields you may have later
  margin?: number; // generic margin (0-1)
  kmcMargin?: number; // KMC-specific margin (0-1)
};
...
const SuppliersPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(`HTTP ${res.status} loading suppliers`);
        }
        ...
        const normalized: SupplierSummary[] = (Array.isArray(list) ? list : [])
          .map((s: any) => ({
            id: String(s?.id ?? s?.key ?? s?.name ?? crypto.randomUUID?.() ?? Math.random()),
            key: s?.key ?? s?.supplier_key ?? s?.supplierKey,
            name: String(s?.name ?? s?.display_name ?? s?.displayName ?? "Unnamed Supplier"),
  ...
  // --- load supplier detail whenever selectedId changes
  useEffect(() => {
    ...
    async function loadDetail() {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/suppliers/${encodeURIComponent(selectedId)}`, {
          headers: { Accept: "application/json" },
        });

        if (!res.ok) {
          throw new Error(`HTTP ${res.status} loading supplier detail`);
        }
  ...
  // --- load KMC pricing preview when in detail mode + KMC selected
  useEffect(() => {
    ...
    async function loadKmcPreview() {
      ...
      try {
        // Try a couple of likely endpoints, use whichever exists.
        const candidates = [
          `${API_BASE}/api/suppliers/${encodeURIComponent(selectedId)}/pricing_preview`,
