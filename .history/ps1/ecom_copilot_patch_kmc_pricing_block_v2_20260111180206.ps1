...
Write-Host "Patching KMC pricing block in SuppliersPage.tsx (v2)..." -ForegroundColor Cyan
...
# Read the whole TSX file
$content = Get-Content -LiteralPath $file -Raw

# Now we ONLY replace the KMC block between the existing markers
$pattern = "(?s)// KMC_PRICING_BLOCK_START.*?// KMC_PRICING_BLOCK_END"

$replacement = @'
// KMC_PRICING_BLOCK_START
// Detail mode for supplier view (includes KMC pricing preview + CSV mapping for KMC)
if (mode === "detail") {
  ...
  return (
    <div className="flex-1 flex flex-col space-y-4">
      ...
      {/* KMC pricing preview table (sample rows) */}
      {isKmc && (
      ...
      {/* KMC CSV upload + mapping + export (UI only v1) */}
      {isKmc && (
        <section className="bg-white rounded-xl shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-xs text-slate-400">
              v1 – mapping UI only (wired to desktop engine for now)
            </div>
          </div>

          {/* Global fee + margin inputs (not wired yet) */}
          ...
          <div className="mt-4 border-t border-slate-200 pt-4">
            ...
            <p className="text-xs text-slate-500 mb-3">
              ...
              the Python pricing engine – you can run the desktop .bat for now.
            </p>
...
if ($content -notmatch $pattern) {
    throw "Could not find KMC_PRICING_BLOCK in $file"
}
...
Write-Host "✅ KMC pricing block updated (v2 mapping UI)." -ForegroundColor Green
