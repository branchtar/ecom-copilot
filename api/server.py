...
# exchange spapi_oauth_code for tokens, and store refresh_token locally for dev.
...
async def amazon_connect_callback(request: Request):
    ...
    selling_partner_id = (query_params.get("selling_partner_id") or "").strip()
    spapi_oauth_code = (query_params.get("spapi_oauth_code") or "").strip()

    if not state:
        return {"ok": False, "error": "Missing state", "query_params": query_params}

    if not selling_partner_id:
        return {"ok": False, "error": "Missing selling_partner_id", "query_params": query_params}

    if not spapi_oauth_code:
        return {"ok": False, "error": "Missing spapi_oauth_code", "query_params": query_params}

    # Best-effort decode of the state payload generated by amazon_connect_start().
    state_payload = {}
    try:
    ...
    tenant = (state_payload.get("tenant") or "dev")

    client_id = (os.getenv("AMAZON_SPAPI_CLIENT_ID") or "").strip()
    client_secret = (os.getenv("AMAZON_SPAPI_CLIENT_SECRET") or "").strip()
    redirect_uri = (os.getenv("AMAZON_SPAPI_REDIRECT_URI") or "").strip()

    if not client_id:
        return {"ok": False, "error": "Missing env var AMAZON_SPAPI_CLIENT_ID"}

    if not client_secret:
        return {"ok": False, "error": "Missing env var AMAZON_SPAPI_CLIENT_SECRET"}

    if not redirect_uri:
        return {"ok": False, "error": "Missing env var AMAZON_SPAPI_REDIRECT_URI"}

    token_url = "https://api.amazon.com/auth/o2/token"
    token_form = urlparse.urlencode({
    ...
    try:
        with urlrequest.urlopen(token_req, timeout=30) as resp:
        ...
        return {
            "ok": False,
            "error": "Amazon token exchange failed",
    ...
    if not refresh_token:
        return {
            "ok": False,
            "error": "Amazon token exchange succeeded but no refresh_token was returned",
            "token_payload": token_payload,
        }

    token_store_path = Path(__file__).parent / "amazon_oauth_tokens.json"

    try:
        if token_store_path.exists():
            existing = json.loads(token_store_path.read_text(encoding="utf-8"))
        else:
            existing = {}
    except Exception:
        existing = {}

    existing[selling_partner_id] = {
        "tenant": tenant,
        "selling_partner_id": selling_partner_id,
        "refresh_token": refresh_token,
        "access_token_last_received": access_token,
        "expires_in": expires_in,
        "updated_at": int(time.time()),
        "state_payload": state_payload,
    }

    token_store_path.write_text(json.dumps(existing, indent=2), encoding="utf-8")

    return {
        "ok": True,
        "message": "Amazon OAuth completed and refresh token saved",
        "tenant": tenant,
        "selling_partner_id": selling_partner_id,
        "saved_file": str(token_store_path.name),
    }
