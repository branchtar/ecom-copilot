...
if ($txt -match 'loadDemoSuppliers' -or $txt -match 'Use demo data') {
  Write-Host "Demo/Retry already present. No changes made." -ForegroundColor Yellow
  exit 0
}

# 1) Inject helpers right after FIRST occurrence of list-mode if
$injectLines = @(
'if (mode === "list") {',
...
if ($txt2 -eq $txt) {
  throw 'Could not find list-mode block: if (mode === "list") {'
}

# 2) Replace FIRST error JSX block that contains "Failed to fetch"
$errorLines = @(
'{!loading && error && (',
...
'      <span className="font-mono">{suppliersUrl}</span>',
'    </div>',
...
if ($txt3 -eq $txt2) {
  Write-Host "WARNING: Could not find an error JSX block containing 'Failed to fetch' to replace." -ForegroundColor Yellow
