# Ecom Copilot - Add USPS Pricing Sandbox (RateV4 Priority Mail)
...
Write-Host "  Ecom Copilot - Add USPS Pricing Sandbox " -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($d in @($pyDir, $ps1Dir, $runDir)) {
    if (-not (Test-Path -LiteralPath $d)) {
        New-Item -ItemType Directory -Path $d | Out-Null
        Write-Host "Created folder: $d" -ForegroundColor Green
    } else {
        Write-Host "Folder already exists: $d" -ForegroundColor DarkGray
    }
}

# ------------------------------------------------------------------
# 1) Python USPS Pricing Sandbox GUI
# ------------------------------------------------------------------
$pricingPyPath = Join-Path $pyDir "usps_pricing_sandbox.py"
$pricingPyContent = @'
"""
...
$pricingPyContent | Set-Content -Path $pricingPyPath -Encoding UTF8
Write-Host "Wrote USPS pricing sandbox Python: $pricingPyPath" -ForegroundColor Yellow

# ------------------------------------------------------------------
# 2) Runner PS1 for the pricing sandbox
# ------------------------------------------------------------------
$runnerPs1Path = Join-Path $ps1Dir "ecom_copilot_run_usps_pricing.ps1"
$runnerPs1Content = @'
param(
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    Write-Host "❌ usps_pricing_sandbox.py not found at:" -ForegroundColor Red
...
try {
    ...
    Write-Host "⚠ Could not install 'requests'. You may need to run 'py -m pip install requests' manually." -ForegroundColor Yellow
}
...
$runnerPs1Content | Set-Content -Path $runnerPs1Path -Encoding UTF8
Write-Host "Wrote USPS pricing runner PS1: $runnerPs1Path" -ForegroundColor Yellow

# ------------------------------------------------------------------
# 3) BAT launcher in /run
# ------------------------------------------------------------------
$batPath = Join-Path $runDir "ecom_copilot_usps_pricing.bat"
$batContent = @'
@echo off
...
Write-Host "Wrote USPS pricing BAT launcher: $batPath" -ForegroundColor Yellow

# ------------------------------------------------------------------
# 4) Launch once now
# ------------------------------------------------------------------
Write-Host ""
Write-Host "Launching USPS Pricing Sandbox now..." -ForegroundColor Green
Write-Host ""

powershell -ExecutionPolicy Bypass -File $runnerPs1Path -Root $root

Write-Host ""
Write-Host "Next time, just double-click this BAT to open the USPS pricing sandbox:" -ForegroundColor Green
